...
| `text/brand` | `#b02d50` | `TEXT_FILL` | Функциональный брендовый акцент в тексте: активное или текущее состояние. Scope Figma: Text — переменная показывается только для заливки текста. |
...
| `icon/brand` | `#b02d50` | `SHAPE_FILL + STROKE_COLOR` | Функциональные брендовые иконки активного или текущего состояния. Scope Figma: Shape — переменная показывается только для заливки геометрических форм. Scope Figma: Stroke — переменная показывается только для обводки. |
...
| `border/brand` | `#b02d50` | `STROKE_COLOR` | Брендовая обводка активного, текущего или сфокусированного элемента. Scope Figma: Stroke — переменная показывается только для обводки. |
| `border/brand/strong` | `#992545` | `STROKE_COLOR` | Усиленная брендовая обводка выбранного таба или акцентной поверхности. Scope Figma: Stroke — переменная показывается только для обводки. |
...
| `button/primary/background` | `#b02d50` | `FRAME_FILL` | Фон главной брендовой кнопки в обычном состоянии. Scope Figma: Frame — переменная показывается только для заливки Frame. |
| `button/primary/background/hover` | `#992545` | `FRAME_FILL` | Фон главной брендовой кнопки при hover или pressed. Scope Figma: Frame — переменная показывается только для заливки Frame. |
| `button/primary/border` | `#b02d50` | `STROKE_COLOR` | Обводка главной брендовой кнопки в обычном состоянии. Scope Figma: Stroke — переменная показывается только для обводки. |
| `button/primary/border/hover` | `#992545` | `STROKE_COLOR` | Обводка главной брендовой кнопки при hover или pressed. Scope Figma: Stroke — переменная показывается только для обводки. |
...
| `button/soft/foreground` | `#b02d50` | `SHAPE_FILL + TEXT_FILL + STROKE_COLOR` | Текст и системная иконка мягкой брендовой кнопки. Scope Figma: Text, Shape и Stroke — единый foreground намеренно окрашивает label, fill-иконку и stroke-иконку внутри одного контрола. |
...
| `shape/carousel/active` | `#b02d50` | `SHAPE_FILL` | Активная точка пагинации фотокарусели. Scope Figma: Shape — переменная показывается только для заливки геометрических форм. |
...
| `effect/focus/ring` | `rgba(176, 45, 80, 0.7)` | `EFFECT_COLOR` | Цвет внешнего focus ring интерактивного элемента, реализованного эффектом. Scope Figma: Effects — переменная показывается только для цвета тени или другого эффекта. |
