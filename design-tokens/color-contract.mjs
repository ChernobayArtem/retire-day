...
export const colorCollections = Object.freeze({
  primitives: {
    ...
    description:
      'Сырые значения палитры на стандартной шкале: 0–50 — самые светлые, около 500 — базовые, 700–1000 — тёмные. У alpha-примитивов последнее число означает процент непрозрачности. Служебная коллекция: переменные из неё нельзя применять напрямую к макетам или компонентам.',
