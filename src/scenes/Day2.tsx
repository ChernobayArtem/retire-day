...
function Flower({
  ...
  return (
    <g className={'fl ' + (className ?? '')} style={{ transformOrigin: `${x}px ${base}px` }}>
      <path
        className="stem"
        d={`M${x} ${base} C ${x - 6} ${base - 40}, ${x + 6} ${cy + 50}, ${x} ${cy + 20}`}
      ...
      <g className="petals" style={{ transformOrigin: `${x}px ${cy}px` }}>
        {petals.map((a) => {
          ...
          return (
            <ellipse
              key={a}
              cx={x + Math.cos(rad) * 15}
              cy={cy + Math.sin(rad) * 15}
              rx="11"
              ry="15"
              fill={petal}
              transform={`rotate(${a} ${x + Math.cos(rad) * 15} ${cy + Math.sin(rad) * 15})`}
            />
          )
        })}
...
export default function Day2() {
  return (
    ...
    >
      ...
      <Flower
        x={92}
        petal="var(--color-alias-illustration-rose-419)"
        center="var(--color-alias-illustration-orange-263)"
        className="f1"
      />
      <Flower
        x={215}
        petal="var(--color-alias-illustration-indigo-250)"
        center="var(--color-alias-illustration-orange-263)"
        className="f2"
      />
      <Flower
        x={338}
        petal="var(--color-alias-illustration-orange-281)"
        center="var(--color-alias-illustration-red-750)"
        className="f3"
      />
      <g fill="var(--color-alias-illustration-green-769)">
        ...
        <path
          ...
          d="M274 232 C 290 222 292 210 276 208 C 268 217 267 226 274 232 Z"
        />
      </g>
