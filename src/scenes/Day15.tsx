...
function Firefly({ x, y, dx, dy, dur, delay }: { x: number; y: number; dx: string; dy: string; dur: string; delay: string }) {
  return (
    <g className="fly" style={{ ['--dx' as string]: dx, ['--dy' as string]: dy, animationDuration: dur, animationDelay: delay }}>
      <circle cx={x} cy={y} r="12" fill="var(--color-alias-illustration-green-77)" opacity="0.2" />
      <circle cx={x} cy={y} r="5.5" fill="var(--color-alias-illustration-yellow-182)" opacity="0.34" />
...
export default function Day15() {
  return (
    <svg className="scn s15" viewBox="0 0 430 260" preserveAspectRatio="xMidYMax slice" xmlns="http://www.w3.org/2000/svg">
      <style>{`
      ...
      `}</style>
      <defs><linearGradient id="s15sky" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="var(--color-alias-illustration-blue-194)" /><stop offset="0.55" stopColor="var(--color-alias-illustration-blue-764)" /><stop offset="1" stopColor="var(--color-alias-illustration-blue-917)" /></linearGradient></defs>
      <rect width="430" height="260" fill="url(#s15sky)" />
      <g className="moon">
        <circle cx="112" cy="86" r="22" fill="var(--color-alias-illustration-yellow-364)" opacity=".82" />
        <circle cx="122" cy="79" r="22" fill="var(--color-alias-illustration-blue-583)" />
      </g>
      <path className="star" d="M320 70 l2 6 6 2 -6 2 -2 6 -2 -6 -6 -2 6 -2 z" fill="var(--color-alias-illustration-yellow-91)" />
      ...
      <path d="M0 214 Q 215 226 430 214 L430 260 L0 260 Z" fill="var(--color-alias-illustration-blue-986)" />
