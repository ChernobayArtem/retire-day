...
function Firefly({ x, y, dx, dy, dur, delay }: { x: number; y: number; dx: string; dy: string; dur: string; delay: string }) {
  return (
    <g className="fly" style={{ ['--dx' as string]: dx, ['--dy' as string]: dy, animationDuration: dur, animationDelay: delay }}>
      <circle cx={x} cy={y} r="12" fill="#eaff9c" opacity="0.2" />
      <circle cx={x} cy={y} r="5.5" fill="#efffae" opacity="0.34" />
      <circle cx={x} cy={y} r="2.8" fill="#fbffd0" />
    </g>
  )
}
...
export default function Day15() {
  return (
    <svg className="scn s15" viewBox="0 0 430 260" preserveAspectRatio="xMidYMax slice" xmlns="http://www.w3.org/2000/svg">
      ...
      `}</style>
      <defs><linearGradient id="s15sky" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#e3ecf7" /><stop offset="0.55" stopColor="#7f92c4" /><stop offset="1" stopColor="#41527f" /></linearGradient></defs>
      <rect width="430" height="260" fill="url(#s15sky)" />
      <g className="moon">
        <circle cx="112" cy="86" r="22" fill="#fff3c4" opacity=".82" />
        <circle cx="122" cy="79" r="22" fill="#a8b7d8" />
      </g>
      <path className="star" d="M320 70 l2 6 6 2 -6 2 -2 6 -2 -6 -6 -2 6 -2 z" fill="#fdfbe6" />
      ...
      <path d="M0 214 Q 215 226 430 214 L430 260 L0 260 Z" fill="#2f3f66" />
      <g fill="#3a4d78">
